...
[[nodiscard]] auto to_vma_flags(const BufferCreateInfo& info) noexcept -> VmaAllocationCreateFlags {
    VmaAllocationCreateFlags f = 0;
    if (info.persistent_map || info.memory == MemoryUsage::cpu_to_gpu ||
...
auto Buffer::create(const Allocator& allocator, const BufferCreateInfo& info) -> Result<Buffer> {
    if (info.size == 0) {
        return std::unexpected(
            noted::make_error(noted::ErrorCode::invalid_argument, "Buffer::create: size == 0"));
    }
    ...
    if (auto vr = vmaCreateBuffer(allocator.handle(), &bci, &aci, &raw_buf, &alloc, &alloc_info);
        vr != VK_SUCCESS) {
