...
void image_layout_transition(VkCommandBuffer cb,
                             VkImage image,
...
auto Renderer::create(const Device& device,
                      const Swapchain& swapchain,
                      std::uint32_t frames_in_flight) -> Result<Renderer> {
    if (frames_in_flight == 0) {
        return std::unexpected(
            noted::make_error(noted::ErrorCode::invalid_argument, "frames_in_flight must be >= 1"));
    }
...
auto Renderer::rebind_swapchain(const Device& device, const Swapchain& swapchain) -> Result<void> {
...
auto Renderer::render_frame(const Device& device,
                            const Swapchain& swapchain,
    ...
    if (auto vr = vkWaitForFences(owner_, 1, &fence_h, VK_TRUE, UINT64_MAX); vr != VK_SUCCESS) {
    ...
    auto acq =
        vkAcquireNextImageKHR(owner_, sc_h, UINT64_MAX, acquire_h, VK_NULL_HANDLE, &image_index);
    if (acq == VK_ERROR_OUT_OF_DATE_KHR) {
        return std::unexpected(
            noted::make_error(noted::ErrorCode::gpu_swapchain_out_of_date,
    ...
    vkCmdClearColorImage(
        slot.cb.handle(), image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &color, 1, &range);
    ...
    if (auto vr = vkQueueSubmit2(device.graphics_queue(), 1, &si, fence_h); vr != VK_SUCCESS) {
        return std::unexpected(noted::make_error(
            noted::ErrorCode::gpu_validation_failed,
    ...
    if (pres == VK_ERROR_OUT_OF_DATE_KHR) {
        return std::unexpected(noted::make_error(noted::ErrorCode::gpu_swapchain_out_of_date,
                                                 "vkQueuePresentKHR: VK_ERROR_OUT_OF_DATE_KHR"));
    }
    if (pres == VK_SUBOPTIMAL_KHR || suboptimal_acq) {
        return std::unexpected(noted::make_error(noted::ErrorCode::gpu_swapchain_suboptimal,
...
auto Renderer::render_frame_with(const Device& device,
                                 const Swapchain& swapchain,
    ...
    VkResult acq = VK_SUCCESS;
    {
        ...
        acq = vkAcquireNextImageKHR(
            owner_, sc_h, UINT64_MAX, acquire_h, VK_NULL_HANDLE, &image_index);
    }
    if (acq == VK_ERROR_OUT_OF_DATE_KHR) {
        return std::unexpected(
            noted::make_error(noted::ErrorCode::gpu_swapchain_out_of_date,
