...
auto Image::create(const Allocator& allocator, const ImageCreateInfo& info)
    -> Result<Image> {
    ...
    if (auto vr = vmaCreateImage(allocator.handle(), &ici, &aci,
                                 &raw_img, &alloc, nullptr); vr != VK_SUCCESS) {
        return std::unexpected(noted::make_error(
    ...
    if (auto vr = vkCreateImageView(allocator.device(), &vci, nullptr, &view);
        vr != VK_SUCCESS) {
