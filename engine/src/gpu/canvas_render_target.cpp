...
constexpr VkImageUsageFlags kCanvasUsage =
...
auto CanvasRenderTarget::create(
    ...
    auto img = Image::create(allocator, ImageCreateInfo{
        .format = info.format,
...
auto CanvasRenderTarget::resize(
    const Allocator& allocator,
    VkExtent2D       new_extent) -> Result<void> {
    if (!image_.has_value()) {
        return std::unexpected(noted::make_error(
            noted::ErrorCode::invalid_state,
            "CanvasRenderTarget::resize: canvas is disengaged"));
    }
    ...
    auto fresh = Image::create(allocator, ImageCreateInfo{
...
void CanvasRenderTarget::transition_to(
    VkCommandBuffer       cb,
...
auto CanvasRenderTarget::color_attachment(
    VkClearColorValue   clear,
    VkAttachmentLoadOp  load_op,
    VkAttachmentStoreOp store_op) const noexcept -> VkRenderingAttachmentInfo {
