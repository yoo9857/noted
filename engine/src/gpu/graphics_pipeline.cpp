...
auto GraphicsPipelineBuilder::vertex_input(
...
auto GraphicsPipelineBuilder::depth_test(bool enable, VkCompareOp op)
    -> GraphicsPipelineBuilder& {
    depth_test_ = enable;
    depth_op_   = op;
    return *this;
...
auto GraphicsPipelineBuilder::color_blend_attachment(
    const VkPipelineColorBlendAttachmentState& s) -> GraphicsPipelineBuilder& {
    color_blend_ = s;
    return *this;
}
...
auto GraphicsPipelineBuilder::build(const Device&         device,
    ...
    if (auto vr = vkCreateGraphicsPipelines(device.handle(), VK_NULL_HANDLE,
                                            1, &gpci, nullptr, &raw);
        vr != VK_SUCCESS) {
        return std::unexpected(noted::make_error(
            noted::ErrorCode::gpu_shader_compile_failed,
            std::string{"vkCreateGraphicsPipelines: "} +
