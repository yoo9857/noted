...
auto DescriptorPool::create(const Device& device,
                            const DescriptorPoolCreateInfo& info) -> Result<DescriptorPool> {
    if (info.max_sets == 0) {
    ...
    if (auto vr = vkCreateDescriptorPool(device.handle(), &ci, nullptr, &raw); vr != VK_SUCCESS) {
        return std::unexpected(noted::make_error(
            noted::ErrorCode::gpu_validation_failed,
            std::string{"vkCreateDescriptorPool: "} + std::to_string(static_cast<int>(vr))));
    }
    DescriptorPool p;
...
auto DescriptorPool::allocate(const DescriptorSetLayout& layout) -> Result<VkDescriptorSet> {
    ...
    if (auto vr = vkAllocateDescriptorSets(owner_, &ai, &set); vr != VK_SUCCESS) {
        return std::unexpected(noted::make_error(
            ...
            std::string{"vkAllocateDescriptorSets: "} + std::to_string(static_cast<int>(vr))));
    }
    return set;
}
