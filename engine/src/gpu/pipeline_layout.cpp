...
auto PipelineLayout::create(
    const Device&                                  device,
    std::span<const DescriptorSetLayout* const>    set_layouts,
    std::span<const VkPushConstantRange>           push_ranges) -> Result<PipelineLayout> {
    ...
    if (auto vr = vkCreatePipelineLayout(device.handle(), &ci, nullptr, &raw);
        vr != VK_SUCCESS) {
        return std::unexpected(noted::make_error(
            noted::ErrorCode::gpu_validation_failed,
            std::string{"vkCreatePipelineLayout: "} +
