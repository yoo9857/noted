...
auto upload_image_pixels(
    ...
    // 1. Staging buffer.
    auto staging = Buffer::create(allocator, BufferCreateInfo{
    ...
    auto pool = CommandPool::create(device, info.queue_family,
    ...
    barrier(cb->handle(), dst.handle(), info.aspect,
            info.mip_level, info.base_array_layer, info.layer_count,
            info.current_layout, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    ...
    barrier(cb->handle(), dst.handle(), info.aspect,
            info.mip_level, info.base_array_layer, info.layer_count,
            VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, info.final_layout,
    ...
    if (auto vr = vkCreateFence(device.handle(), &fci, nullptr, &done);
        vr != VK_SUCCESS) {
        return std::unexpected(noted::make_error(
