...
[[nodiscard]] auto pick_surface_format(VkPhysicalDevice physical,
    ...
    const VkFormat preferred = prefer_srgb ? VK_FORMAT_B8G8R8A8_SRGB : VK_FORMAT_B8G8R8A8_UNORM;
    ...
    const VkFormat fallback = prefer_srgb ? VK_FORMAT_B8G8R8A8_UNORM : VK_FORMAT_B8G8R8A8_SRGB;
    for (const auto& f : formats) {
        if (f.format == fallback && f.colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR) {
            return f;
        }
    }
...
[[nodiscard]] auto pick_present_mode(VkPhysicalDevice physical,
                                     VkSurfaceKHR surface,
                                     bool allow_mailbox,
...
[[nodiscard]] auto build(VkPhysicalDevice physical,
                         VkDevice device,
                         VkSurfaceKHR surface,
                         ...
                         SwapchainSummary& out_summary) -> noted::Result<VkSwapchainKHR> {
    ...
    if (auto vr = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physical, surface, &caps);
        ...
        return std::unexpected(
            noted::make_error(noted::ErrorCode::gpu_validation_failed,
                              std::string{"vkGetPhysicalDeviceSurfaceCapabilitiesKHR: "} +
...
[[nodiscard]] auto fetch_images_and_views(VkDevice device,
...
auto Swapchain::create(const PhysicalDevice& physical,
                       const Device& device,
    ...
    auto raw = build(physical.handle(),
                     device.handle(),
                     surface.handle(),
                     window_extent,
                     cfg,
    ...
    if (auto r = fetch_images_and_views(
            device.handle(), s.handle_, s.summary_.color_format, s.images_, s.views_);
        !r) {
...
auto Swapchain::recreate(const PhysicalDevice& physical,
                         const Surface& surface,
                         VkExtent2D window_extent,
    ...
    handle_ = *raw;
    if (auto r = fetch_images_and_views(owner_, handle_, summary_.color_format, images_, views_);
        !r) {
        return std::unexpected(std::move(r).error());
    }
    return {};
