...
void enqueue_write(std::vector<VkWriteDescriptorSet>& writes,
...
auto DescriptorWriter::write_combined_image_sampler(std::uint32_t binding,
                                                    VkImageView view,
                                                    VkSampler sampler,
...
auto DescriptorWriter::write_uniform_buffer(std::uint32_t binding,
                                            VkBuffer buffer,
                                            VkDeviceSize offset,
                                            VkDeviceSize range,
    ...
    enqueue_write(
        writes_, set_.handle(), binding, array_element, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1);
    pending_.push_back({InfoKind::buffer, buffer_infos_.size() - 1});
    return *this;
}

auto DescriptorWriter::write_storage_buffer(std::uint32_t binding,
                                            VkBuffer buffer,
                                            VkDeviceSize offset,
                                            VkDeviceSize range,
                                            std::uint32_t array_element) -> DescriptorWriter& {
    ...
    enqueue_write(
        writes_, set_.handle(), binding, array_element, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1);
    pending_.push_back({InfoKind::buffer, buffer_infos_.size() - 1});
    return *this;
}

auto DescriptorWriter::write_storage_image(std::uint32_t binding,
                                           VkImageView view,
                                           VkImageLayout layout,
                                           std::uint32_t array_element) -> DescriptorWriter& {
    if (image_infos_.capacity() < kReserve) {
    ...
    enqueue_write(
        writes_, set_.handle(), binding, array_element, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1);
    pending_.push_back({InfoKind::image, image_infos_.size() - 1});
    return *this;
}
...
void DescriptorWriter::commit() {
    ...
    vkUpdateDescriptorSets(
        set_.device(), static_cast<std::uint32_t>(writes_.size()), writes_.data(), 0, nullptr);
    writes_.clear();
    image_infos_.clear();
    buffer_infos_.clear();
    pending_.clear();
